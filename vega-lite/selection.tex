...
\cref{}\todo{PointSelectionFig} illustrates how points are highlighted in a scatterplot
...
\cref{}\todo{IntervalSelections(a)}, to depict the selected interval. Users can
...
\cref{}\todo{PointSelections(c)} demonstrates how \texttt{mouseover} events are used to
...
\texttt{size}). \cref{}\todo{PointSelections(d, e)} demonstrate how \emph{project} can
...
the shift key pressed. For example, in \cref{}\todo{PointSelections(b)}, additional
...
\cref{}\todo{PointSelections(c)}, however, specifies a custom \texttt{mouseover} event.
...
transforms, enabling movement of brushed regions (\cref{}\todo{IntervalSelections(b)})
or panning of the visualization when bound to scale functions
(\cref{}\todo{PanZoomGrid}).
...
selection or not. For example, as shown in \cref{}\todo{PointSelections}, the fill
...
viewports, including panning \& zooming (\cref{}\todo{PanZoomGrid}) and
overview\,+\,detail (\cref{}\todo{ODIndexChart(a)}).
...
For example, the position of the red rule in \cref{}\todo{ODIndexChart(b)} is set to
...
Consider \cref{}\todo{ResolveSelections(a)}, which illustrates how a scatterplot matrix
...
example, this would produce the interaction shown in \cref{}\todo{ResolveSelections
...
within every brush (\emph{intersect}) as shown in \cref{}\todo{ResolveSelections(c,
